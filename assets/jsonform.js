...
jsonform.fieldTemplate = function(inner) {
  return '<div id="<%= node.name %>"' +
    ...
    '<% }%>' +
    'class="form-field form-field--half display-no jsonform-error-<%= keydash %>' +
    ...
    '">' +
    '<% if (!elt.notitle) { %>' +
      '<label class="form-field-title" for="<%= node.id %>"><%= node.title ? node.title : node.name %></label>' +
    '<% } %>' +
    '<div class="controls">' +
...
var inputFieldTemplate = function (type) {
  return {
    'template': '<input type="' + type + '" ' +
      'class=\'form-field-input form-field-text jsonformselector <%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>\'' +
      ...
      '<%= (node.schemaElement && (node.schemaElement.min > 0) ? " min=\'" + node.schemaElement.min + "\'" : "") %>' +
...
jsonform.elementTypes = {
  ...
  'root': {
    'template': '<div id="formulariopre"><%= children %></div>'
  },
  ...
  'select':{
    'template':'<span class="form-field-select-wrapper">'+
      '<select name="<%= node.name %>" id="<%= id %>"' +
      'class=\'form-field-select<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>\'' +
  ...
  'fieldset': {
    'template': '<fieldset style="margin: 16px" class="form-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      '<% if (id) { %> id="<%= id %>"<% } %>' +
      '>' +
  ...
  'submit':{
    'template':'<input type="submit" <% if (id) { %> id="<%= id %>" <% } %> id="btn-json-form" class="button-primary display-no <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"<%= (node.disabled? " disabled" : "")%>/>'
  },
  'button':{
    ...
    'template': '<fieldset class="tab-container <%= elt.htmlClass?elt.htmlClass:"" %>">' +
      ...
      '<div class="tabbable">' +
        '<div class="form-group<%= node.formElement.hideMenu ? " hide" : "" %>">' +
          '<% if (!elt.notitle) { %><label class="form-field-title" for="<%= node.id %>"><%= node.title ? node.title : node.name %></label><% } %>' +
